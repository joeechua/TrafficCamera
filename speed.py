...
class VehicleCounter (object):
	def __init__(self, shape, div, fps, samples=0):
		self.height, self.width = shape
		self.divider = div
		self.is_horizontal = True
		self.pass_side = -1
		self.vector_angle_min = 100
		self.vector_angle_max = 180
	...
	def is_valid_vector (a, angle_min, angle_max):
		...
		#distance <= 110 and 
		#return (distance <= 10 and angle > angle_min and angle < angle_max)
		return distance <= 80
		...
		for vehicle in self.vehicles:
			if not vehicle.counted and self.is_past_divider(vehicle.last_position):
				if self.sample_num == 0:
					...
					vehicle.speed = vehicle.speed*1.609344 #convert to kmh
